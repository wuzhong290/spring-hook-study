...
import com.demo.javabase.nio.buffer.BufferQueue;
...
public class DemoConnectionFactory {
    ...
    public DemoConnection make(SocketChannel channel) throws IOException {
        Socket socket = channel.socket();
        socket.setReceiveBufferSize(socketRecvBuffer);
        socket.setSendBufferSize(socketSendBuffer);
        socket.setTcpNoDelay(true);
        socket.setKeepAlive(true);
        DemoConnection connection = new DemoConnection(channel);
        connection.setPacketHeaderSize(packetHeaderSize);
        connection.setMaxPacketSize(maxPacketSize);
        connection.setWriteQueue(new BufferQueue(writeQueueCapcity));
